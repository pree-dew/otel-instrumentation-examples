...
func main() {
	...
	shutdown, err := instrumentation.Run(ctx, "notes-server")
	if err != nil {
	...
	db, err = otelsql.Open("pgx", "postgres://postgres:pswd@localhost:5434/postgres", otelsql.WithAttributes(semconv.DBSystemPostgreSQL))
	if err != nil {
